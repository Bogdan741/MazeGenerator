...
{
    std::vector<bool> visited_a(false, spanningTree.size());
    std::vector<uint32_t> parents_a(-1, spanningTree.size());
    std::vector<double> globalDist_a(-1, spanningTree.size());
    std::vector<double> localDist_a(-1, spanningTree.size());
    ...
    while (!notTestedNodes_a.empty())
    {
        ...
        for (auto const vertex : spanningTree[current])
        {
            ...
            float fPossLoverGoal = localDist_a[current] + distance(current, end);
            if (fPossLoverGoal < localDist_a[vertex])
            {
