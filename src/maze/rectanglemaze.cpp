...
uint32_t RectangleMaze::GetIndex(uint32_t y, uint32_t x)
{
    return y * m_width + x;
}
...
void RectangleMaze::InitializeMaze()
{
    ...
    {
        adjucenyList[GetIndex(0, i)].push_back({-1, std::make_shared<Line>(i, 0, i + 1, 0)});
        adjucenyList[GetIndex(m_height - 1, i)].push_back({-1, std::make_shared<Line>(i, m_height, i + 1, m_height)});
    }
    ...
    {
        if (i != 0)
            adjucenyList[GetIndex(i, 0)].push_back({-1, std::make_shared<Line>(0, i, 0, i - 1)});
        if (i != m_height)
            adjucenyList[GetIndex(i, m_width - 1)].push_back({-1, std::make_shared<Line>(m_width, i, m_width, i - 1)});
    }
...
std::vector<Connection> RectangleMaze::GetDrawPath(std::vector<std::pair<uint32_t, uint32_t>> & path) 
{
...
std::pair<uint32_t, uint32_t> RectangleMaze::GetPosition(uint32_t u)
{
